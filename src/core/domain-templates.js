...
export function domainPayloadValue(group, operation, { revision = 0, documentDigest = DIGEST_A } = {}) {
  ...
  const templates = {
    ...
    'AEC.createToolset': {
      input: {
        name: 'Local QA', tools: ['cloud', 'arrow'], sourceSha256: documentDigest,
      },
    },
    'AEC.createReviewSession': { input: { workspaceId: 'workspace-1', participants: ['author', 'reviewer'], sourceSha256: documentDigest } },
    'AEC.measurementToolset': { input: { sourceSha256: documentDigest } },
    'AEC.createMarkup': { input: { type: 'issue', page: 1, status: 'open', properties: {}, sourceSha256: documentDigest } },
    'AEC.listMarkups': { query: { status: 'open', sourceSha256: documentDigest, expectedRevision: revision } },
    'AEC.createCustomColumn': {
      input: {
        id: 'column-1',
        name: 'Total',
        formula: 'length * width + 2',
        ...(documentDigest === undefined ? {} : { sourceSha256: documentDigest }),
      },
    },
    'AEC.evaluateCustomColumn': {
      columnId: 'column-1',
      values: { length: 3, width: 4 },
      ...(documentDigest === undefined ? {} : { sourceSha256: documentDigest }),
      options: { expectedRevision: revision },
    },
    'AEC.createSpace': { input: { name: 'Room 1', kind: 'space', page: 1, points: [{ x: 0, y: 0 }, { x: 100, y: 0 }, { x: 0, y: 100 }], sourceSha256: documentDigest } },
    'AEC.createDrawingSet': {
      input: {
        name: 'Issued set',
        sheets: ['sheet-1'],
        initialLog: { revisionLabel: 'Issued', date: '2099-01-01' },
        sourceSha256: documentDigest,
      },
    },
    'AEC.createSheet': {
      input: { number: 'A100', title: 'Plan', tags: ['sheet'], page: 1, sourceSha256: documentDigest },
      options: { expectedRevision: revision },
    },
    'AEC.createRevisionOverlay': {
      input: {
        fromDigest: documentDigest === DIGEST_A ? DIGEST_B : DIGEST_A,
        toDigest: documentDigest,
        sheetId: 'sheet-1',
        sourceSha256: documentDigest,
      },
    },
    'AEC.createBatchPlan': {
      input: {
        kind: 'slip-sheet',
        pairs: [{ from: 'sheet-old', to: 'sheet-new' }],
        sourceSha256: documentDigest,
      },
    },
    'AEC.calibrateGeoPage': {
      input: {
        id: 'geo-1',
        page: 1,
        origin: { x: 10, y: 20 },
    ...
    'AEC.takeoff': { input: { measurementIds: ['measurement-1'], group: 'default', sourceSha256: documentDigest }, options: { expectedRevision: revision } },
    'collaboration.createProject': { input: { name: 'Local project', offline: true } },
    'collaboration.createRevision': { input: { label: 'R1', sourceSha256: documentDigest } },
    'collaboration.transitionRevision': { revisionId: 'revision-1', nextStatus: 'issued', sourceSha256: documentDigest },
