...
export async function runPrepressCommand(application, command, document, stdout, signal, runtime) {
  ...
  if (command.operation === 'preflight') {
    const xml = serializePreflightReportXml(result);
    if (result.profile.id !== command.profile) {
      throw new HostError(
        'INVALID_PREFLIGHT_REPORT',
        'The preflight report does not match the requested fixed profile.',
        502,
      );
    }
    output = command.format === 'xml' ? xml : result;
  }
