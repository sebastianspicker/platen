...
export function pdfkitTargetedSections(state, context) {
  ...
  return `
        ...
        <details>
          ...
          <label class="field-label" for="pdfkit-existing-annotation-index">Inert annotation on selected page</label>
          <select id="pdfkit-existing-annotation-index" ${pdfkitExistingAnnotationReady ? '' : 'disabled'}>
            ${pdfkitExistingAnnotations.length ? pdfkitExistingAnnotations.map((annotation) => `<option value="${annotation.annotationIndex}" ${String(annotation.annotationIndex) === String(state.pdfkitExistingAnnotationIndex) ? 'selected' : ''}>${escapeHtml(annotation.subtype)} · page annotation ${annotation.annotationIndex}</option>`).join('') : '<option value="">No supported annotations on this page</option>'}
          </select>
          <label class="field-label" for="pdfkit-existing-annotation-contents">Replacement contents</label>
          <textarea id="pdfkit-existing-annotation-contents" maxlength="1024" rows="3" ${pdfkitExistingAnnotationReady ? '' : 'disabled'}>${escapeHtml(state.pdfkitExistingAnnotationContents ?? '')}</textarea>
          <div class="numeric-grid" aria-label="Replacement annotation rectangle in PDF points">
            <label>X <input id="pdfkit-existing-annotation-x" type="number" step="0.1" value="${escapeHtml(state.pdfkitExistingAnnotationRect?.x ?? 36)}" ${pdfkitExistingAnnotationReady ? '' : 'disabled'} /></label>
            <label>Y <input id="pdfkit-existing-annotation-y" type="number" step="0.1" value="${escapeHtml(state.pdfkitExistingAnnotationRect?.y ?? 36)}" ${pdfkitExistingAnnotationReady ? '' : 'disabled'} /></label>
            <label>Width <input id="pdfkit-existing-annotation-width" type="number" min="0.1" step="0.1" value="${escapeHtml(state.pdfkitExistingAnnotationRect?.width ?? 180)}" ${pdfkitExistingAnnotationReady ? '' : 'disabled'} /></label>
            <label>Height <input id="pdfkit-existing-annotation-height" type="number" min="0.1" step="0.1" value="${escapeHtml(state.pdfkitExistingAnnotationRect?.height ?? 80)}" ${pdfkitExistingAnnotationReady ? '' : 'disabled'} /></label>
          </div>
          ...
          <p class="field-help">Contents remain private. Updating may regenerate the target appearance stream. Removal is a narrow selective-sanitization subset: after reopen, the selected page/index descriptor occurrence must be omitted and every other ordered reachable annotation descriptor must match. Raw annotation identity must be unique across the whole document. That PDFKit removal does not claim orphan-byte scrubbing or prior-revision removal. Flattening is narrower still: it accepts only the sole annotation in the document, a printed square with one tiny resource-free normal appearance on an unrotated page. It promotes that appearance into page content and emits a closed rewrite without the annotation object or prior revisions. Unsupported appearance states, resources, actions, popups, widgets, filters, and graphs fail closed. Sidecar review annotations are never embedded implicitly.</p>
        </details>`;
}
