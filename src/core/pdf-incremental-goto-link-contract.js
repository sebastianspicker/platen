...
function validArtifactIdentity(artifact, context) {
  return OPAQUE_ID_PATTERN.test(artifact.id ?? '') && artifact.id !== context.documentId
    && artifact.documentId === context.documentId;
}

function validArtifactDisplayName(artifact) {
  return typeof artifact.displayName === 'string' && artifact.displayName.length >= 1
    && artifact.displayName.length <= 240
    && !/[\u0000-\u001f\u007f]/.test(artifact.displayName);
}

function validArtifactMetadata(artifact) {
  return artifact.mediaType === 'application/pdf'
    && Number.isSafeInteger(artifact.size) && artifact.size >= 64
    && artifact.size <= 129 * 1024 * 1024;
}

function validArtifactBinding(artifact, context) {
  return SHA256.test(artifact.sha256 ?? '') && artifact.sha256 !== context.sourceSha256
...
function validArtifact(artifact, context) {
  return exactObject(artifact, [
    ...
    'createdAt',
  ]) && validArtifactIdentity(artifact, context)
    && validArtifactDisplayName(artifact)
    && validArtifactMetadata(artifact)
    && validArtifactBinding(artifact, context);
}

function validOperationHeader(operation) {
  return operation.schemaVersion === 1 && OPAQUE_ID_PATTERN.test(operation.id ?? '')
    && operation.type === 'pdf-incremental-goto-link'
    && typeof operation.completedAt === 'string'
    && !Number.isNaN(Date.parse(operation.completedAt));
}

function validOperationInput(operation, context) {
  return Array.isArray(operation.inputs) && operation.inputs.length === 1
...
function validOperationParameters(operation, context) {
  return exactObject(operation.parameters, ['profile', 'sourcePage', 'targetPage', 'rect'])
...
function validOperationExpectedShape(operation) {
  return exactObject(operation.expected, [
    'pageCount', 'sourceUnchanged', 'sourcePrefixPreserved',
    'classicIncrementalRevisionAppended', 'rasterized',
  ]);
}

function validOperationPageCountBinding(context, pageCount) {
  return Number.isSafeInteger(pageCount) && pageCount >= 1 && pageCount <= 100
    && context.request.sourcePage <= pageCount && context.request.targetPage <= pageCount;
}

function validOperationExpectedFlags(operation) {
  return operation.expected.sourceUnchanged === true
...
function validOperationValidation(operation, artifact, pageCount) {
  return exactObject(operation.validation, ['passed', 'validators', 'pageCount', 'outputSha256'])
...
function validOperation(operation, artifact, context) {
  ...
  return exactObject(operation, [
    ...
    'completedAt',
  ]) && validOperationHeader(operation)
    && validOperationInput(operation, context)
    && validOperationParameters(operation, context)
    && validOperationExpectedShape(operation)
    && validOperationPageCountBinding(context, pageCount)
    && validOperationExpectedFlags(operation)
    && validOperationValidation(operation, artifact, pageCount);
}
...
function validResultIdentity(result, context) {
...
function validResultLink(result, context) {
  return exactObject(result.link, ['sourcePage', 'targetPage', 'rect'])
...
function validResultEvidence(result) {
  return exactObject(result.evidence, EVIDENCE_KEYS)
    && EVIDENCE_KEYS.every((key) => result.evidence[key] === true);
}

function validResultOperation(result, context) {
  return validOperation(result.artifact.operation, result.artifact, context);
}

export function validateIncrementalGoToLinkResult(result, context) {
  if (!validResultIdentity(result, context)) invalidResult();
  if (!validArtifact(result.artifact, context)) invalidResult();
  if (!validResultLink(result, context)) invalidResult();
  if (!validResultEvidence(result)) invalidResult();
  if (!sameList(result.limitations, INCREMENTAL_GOTO_LINK_LIMITATIONS)) invalidResult();
  if (!validResultOperation(result, context)) invalidResult();
  return result;
}
