...
export function createPdfKitMutationOperations({ state, showError, confirm, ready, runArtifact }) {
  ...
  async function runPdfKitTargetedMutation(kind) {
    ...
    const annotationProperties = kind === 'annotation-properties';
    const verb = kind === 'form-fill' ? 'fill the selected form field'
      : kind === 'annotation-update' ? 'update the selected annotation'
        : annotationProperties ? 'update only the selected Square annotation bounds and border color'
          : 'remove the selected annotation';
    const confirmation = selectiveRemoval ? 'Create a separate PDF and remove this selected reachable inert annotation occurrence? The helper compares the ordered raw reachable annotation descriptors before and after reopen. This does not scrub orphan bytes, prior revisions, or other hidden data. The source stays unchanged.' : annotationProperties ? 'Create a separate PDFKit-derived PDF and update only this exact source-bound Square annotation’s bounds and border color? The helper reopens the result, verifies the raw color, and preserves every other annotation descriptor. Signed, encrypted, active-action, XFA, stale-locator, and unsupported targets fail closed. The source stays unchanged.' : `Create a separate PDFKit-derived PDF and ${verb}? Signed, encrypted, active-action, XFA, stale-locator, and unsupported targets fail closed. The source stays unchanged.`;
    if (!confirm(confirmation)) return;
    await runArtifact({ method: 'runPdfKitTargetedMutation', mutation, resultKey: 'pdfkitMutationResult', busyAction: selectiveRemoval ? 'Creating and validating a selective annotation-removal copy…' : annotationProperties ? 'Creating and validating a source-bound Square annotation-property copy…' : `Creating a source-bound ${kind} copy…`, message: (result) => selectiveRemoval ? `${result.artifact.displayName} created with one verified reachable annotation removed. The source is unchanged.` : annotationProperties ? `${result.artifact.displayName} created with verified Square annotation bounds and border color. The source is unchanged.` : `${result.artifact.displayName} created from the exact inspected source locator. The source is unchanged.` });
  }
