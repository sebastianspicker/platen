...
export function normalizeCertificateSignatureRequest(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value) || Object.getPrototypeOf(value) !== Object.prototype
    || ![8, 9].includes(Object.keys(value).length) || Object.keys(value).some((key) => !['profile', 'sourceSha256', 'certificateSha256', 'page', 'fieldName', 'reason', 'location', 'contact', 'placeholderBytes'].includes(key)) || value.profile !== PDF_SIGNATURE_CONTAINER_PROFILE || !SHA256.test(value.sourceSha256 ?? '') || (Object.hasOwn(value, 'certificateSha256') && !SHA256.test(value.certificateSha256 ?? ''))
  ...
  return Object.freeze({ profile: PDF_SIGNATURE_CONTAINER_PROFILE, sourceSha256: value.sourceSha256, ...(Object.hasOwn(value, 'certificateSha256') ? { certificateSha256: value.certificateSha256 } : {}), page: value.page, fieldName: value.fieldName, reason: value.reason, location: value.location, contact: value.contact, placeholderBytes: value.placeholderBytes });
}
