...
import { registerAccessibilityReviewValidatorTests } from './support/accessibility-review-validator-tests.js';
...
function assertReSignedReviewFailure(report, mutate, message) {
  const changed = resignReview(report, mutate);
  assert.throws(
    () => validateAccessibilityReviewReport(changed, {
      expectedSourceDigest: report.sourceDigest,
      requireTrustedIssue: false,
    }),
    { code: 'ACCESSIBILITY_REVIEW_INVALID', message, status: 502 },
  );
}
...
test('accessibility remediation plan validation preserves identity and rejects forged bindings', async () => {
  const report = await fixture().service.review(documentId);
  assert.equal(validateAccessibilityReviewReport(report, { expectedSourceDigest: report.sourceDigest }), report);

  const fixedCandidateTarget = resignReview(report, (changed) => {
    changed.remediationPlan.candidates[0].target = structuredClone(changed.evidence.imageTargets[0]);
  });
  assert.throws(
    () => validateAccessibilityReviewReport(fixedCandidateTarget, { expectedSourceDigest: report.sourceDigest }),
    {
      code: 'ACCESSIBILITY_REVIEW_INVALID',
      message: 'Fixed metadata remediation candidates cannot contain an image target.',
      status: 502,
    },
  );

  const forgedCandidateTarget = resignReview(report, (changed) => {
    const candidate = changed.remediationPlan.candidates.find(({ target }) => target);
    candidate.target = { ...candidate.target, locator: 'b'.repeat(64) };
  });
  assert.throws(
    () => validateAccessibilityReviewReport(forgedCandidateTarget, { expectedSourceDigest: report.sourceDigest }),
    {
      code: 'ACCESSIBILITY_REVIEW_INVALID',
  ...
  const forgedEvidenceDigest = resignReview(report, () => {});
  forgedEvidenceDigest.remediationPlan.reviewEvidenceSha256 = 'c'.repeat(64);
  assert.throws(
    () => validateAccessibilityReviewReport(forgedEvidenceDigest, { expectedSourceDigest: report.sourceDigest }),
    {
      code: 'ACCESSIBILITY_REVIEW_INVALID',
...
registerAccessibilityReviewValidatorTests({ fixture, assertReSignedReviewFailure, resignReview });
