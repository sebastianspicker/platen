...
test('certificate-sign command forwards source/certificate metadata and publishes exclusively', async () => {
  ...
  await runCertificateSignCommand(application, { page: 2, fieldName: 'Signature', reason: '', location: '', contact: '', placeholderBytes: 4096, certificateSha256: 'b'.repeat(64), output: '/tmp/signed.pdf' }, { id: 'doc', sha256: 'a'.repeat(64) }, { write() {} }, undefined, { cancelled() {}, copyExclusive: async (...args) => copied.push(args), emit: async (_out, value) => emitted.push(value) });
  assert.equal(calls[0][0], 'doc'); assert.equal(calls[0][1].sourceSha256, 'a'.repeat(64)); assert.equal(calls[0][2].certificateSha256, 'b'.repeat(64)); assert.deepEqual(copied, [['/private/artifact.pdf', '/tmp/signed.pdf']]); assert.equal(emitted[0].limitations[0], 'bounded');
});
