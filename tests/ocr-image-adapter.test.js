...
test('OCR image adapter pins ImageMagick executable and operation names', async () => { const calls = []; const adapter = new OcrImageAdapter({ registry: { probe: async () => ({ executable: '/engines/magick' }) }, runner: async (call) => { calls.push(call); return {}; } }); await adapter.execute('cleanup', base); assert.equal(calls[0].executable, '/engines/magick'); await assert.rejects(adapter.execute('evil', base), TypeError); });
