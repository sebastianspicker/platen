...
function page(value) { if (!Number.isSafeInteger(value) || value < 1 || value > 10_000) fail('INVALID_REDACTION_PLAN', 'Target page must be an integer from 1 through 10000.'); return value; }
...
function validMark(mark) {
  ...
  return target && typeof mark.id === 'string' && ID.test(mark.id) && Number.isSafeInteger(mark.page) && mark.page > 0 && mark.page <= 10_000
...
function canonicalTimestamp(value) { if (typeof value !== 'string' || value.length > 100) return false; const parsed = new Date(value); return !Number.isNaN(parsed.valueOf()) && parsed.toISOString() === value; }
...
export class RedactionPlanService {
  ...
  async createPlan(documentId, request, { signal } = {}) {
    ...
    try {
      ...
      result = freeze({ plan: publicPlan(record), revision: snapshot.revision });
    } catch (error) { operationError = error; }
    try { await this.#documents.cleanupJob(workspace); } catch (error) { cleanupError = error; }
    if (cleanupError) throw new HostError('REDACTION_PLAN_CLEANUP_FAILED', 'Redaction plan processing could not clean its private workspace.', 500, { cause: operationError ? new AggregateError([operationError, cleanupError]) : cleanupError });
    if (operationError) { this.#cancelled(signal, operationError); throw operationError; }
    return result;
  }
  async applyPlan(documentId, request, { signal } = {}) {
    ...
    try {
      ...
      } finally { excerpt = null; try { await this.#documents.cleanupJob(workspace); } catch (error) { cleanupError = error; } }
      ...
      if (cleanupError) {
        ...
        if (artifact?.id) { try { await this.#documents.deleteArtifact(artifact.id); } catch (cause) { revokeError = cause; } }
        const causes = [cleanupError]; if (revokeError) causes.push(revokeError);
        throw new HostError('REDACTION_PLAN_CLEANUP_FAILED', 'Redaction application could not clean its private workspace or trusted artifact.', 500, { cause: causes.length > 1 ? new AggregateError(causes) : causes[0] });
      }
      await this.#rollback(artifact, error); this.#cancelled(signal, error); throw error;
  ...
  #applyRequest(value) { if (!exact(value, ['schemaVersion', 'profile', 'sourceSha256', 'expectedWorkspaceRevision', 'planId', 'planSha256', 'markIds']) || value.schemaVersion !== 1 || value.profile !== APPLY_PROFILE || !SHA256.test(value.sourceSha256) || !Number.isSafeInteger(value.expectedWorkspaceRevision) || value.expectedWorkspaceRevision < 0 || typeof value.planId !== 'string' || !ID.test(value.planId) || !SHA256.test(value.planSha256) || !Array.isArray(value.markIds) || value.markIds.length < 1 || value.markIds.length > 64 || new Set(value.markIds).size !== value.markIds.length || value.markIds.some((id) => typeof id !== 'string' || !ID.test(id))) fail('INVALID_REDACTION_APPLICATION', 'Redaction application request must use the exact versioned contract.'); return freeze({ ...value, markIds: freeze([...value.markIds]) }); }
  ...
  #id(prefix) { const value = this.#idFactory(prefix); if (typeof value !== 'string' || !ID.test(value)) fail('INVALID_PLAN_ID', 'Redaction plan identifiers must be opaque bounded values.', 500); return value; }
  ...
  #resolve(snapshot, request) {
    ...
    const strict = exact(record, keys) && typeof record.id === 'string' && ID.test(record.id) && record.type === 'redaction-plan'
      && record.profile === PLAN_PROFILE && record.schemaVersion === 1 && record.status === 'proposed-not-applied'
      && canonicalTimestamp(record.createdAtLocal)
