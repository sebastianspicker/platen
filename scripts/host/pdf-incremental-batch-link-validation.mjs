...
export function assertIncrementalBatchLinkProof(proof, sourceLength, outputLength, request) {
  ...
  const linksValid = Array.isArray(proof?.links) && proof.links.length === request.links.length
  ...
  const valid = keys.length === KEYS.length && keys.every((key, index) => key === KEYS[index])
    ...
    && integer(sourceLength, 5, MAX_INCREMENTAL_BATCH_LINK_SOURCE_BYTES)
    ...
    && integer(proof.appendedXrefOffset, sourceLength, outputLength - 1) && linksValid
    && Array.isArray(proof.updatedPageObjectNumbers) && proof.updatedPageObjectNumbers.length >= 1
    && proof.updatedPageObjectNumbers.every((value) => integer(value, 1, proof.effectiveSize - 1))
    && Array.isArray(proof.updatedObjectNumbers) && proof.updatedObjectNumbers.length >= 1
    && proof.updatedObjectNumbers.every((value) => integer(value, 1, proof.effectiveSize - 1))
    && integer(proof.effectiveSize, 2, 999_999) && proof.rootPreserved === true
