...
function presetData(payload) {
  try {
    if (!payload || Object.getPrototypeOf(payload) !== Object.prototype) return null;
    ...
    if (keys.length !== 3 || keys.some((key) => typeof key !== 'string'
      ...
      || !['preset', 'sha256', 'sourceId'].includes(key))) return null;
...
export function presetPayload(payload) {
  ...
  if (!data || typeof data.preset !== 'string'
    || !OPAQUE_ID.test(data.sourceId) || !SHA256.test(data.sha256)) {
...
export function sourcePayload(payload) {
  if (!payload || Object.getPrototypeOf(payload) !== Object.prototype
    || Object.keys(payload).sort().join(',') !== 'sha256,sourceId'
    || !OPAQUE_ID.test(payload.sourceId) || !SHA256.test(payload.sha256)) {
    invalidOperation('Automation operation payload is invalid.');
  }
...
export function ocrPayload(payload) {
  if (!payload || Object.getPrototypeOf(payload) !== Object.prototype
    || Object.keys(payload).sort().join(',') !== 'cleanupPreset,language,segmentation,sha256,sourceId,userDictionary'
    || !OPAQUE_ID.test(payload.sourceId) || !SHA256.test(payload.sha256)
    || typeof payload.language !== 'string'
    || payload.language.split('+').some((token) => !OCR_LANGUAGE.test(token))) {
    invalidOperation('Automation OCR operation payload is invalid.');
  }
...
export function outputIntentPayload(payload) {
  if (!payload || Object.getPrototypeOf(payload) !== Object.prototype
    || Object.keys(payload).sort().join(',') !== 'profile,sha256,sourceId'
    || payload.profile !== OUTPUT_INTENT_PROFILE
    || !OPAQUE_ID.test(payload.sourceId) || !SHA256.test(payload.sha256)) {
    invalidOperation('Automation OutputIntent operation payload is invalid.');
  }
  return Object.freeze({
...
export function fullPageRedactionPayload(payload) {
  if (!payload || Object.getPrototypeOf(payload) !== Object.prototype
    || Object.keys(payload).sort().join(',') !== 'pages,sha256,sourceId'
    || !OPAQUE_ID.test(payload.sourceId) || !SHA256.test(payload.sha256)) {
    invalidOperation('Automation operation payload is invalid.');
  }
  if (!Array.isArray(payload.pages) || payload.pages.length < 1
    || payload.pages.length > MAX_FULL_PAGE_REDACTION_PAGES) {
    invalidOperation('Automation full-page redaction operation payload is invalid.');
  }
  ...
  for (const page of payload.pages) {
    ...
    if (page <= previousPage) {
      invalidOperation('Automation full-page redaction pages must be unique and ascending.');
    }
    previousPage = page;
