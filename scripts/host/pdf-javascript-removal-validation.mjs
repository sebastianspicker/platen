...
export function assertPdfJavaScriptRemovalProof(proof, sourceBytes, outputBytes, request) {
  ...
  if (keys.length !== KEYS.length || !keys.every((key, index) => key === KEYS[index])
    || proof.profile !== PDF_JAVASCRIPT_REMOVAL_PROFILE || request.profile !== proof.profile
    ...
    || !['open-action', 'names'].includes(proof.removedLocus) || !Number.isSafeInteger(proof.removedObjectCount)
    || proof.removedObjectCount !== (proof.removedLocus === 'names' ? 2 : 1)
    || !['closedClassicRevision', 'priorRevisionsAbsent', 'javascriptSurfacesAbsent', 'removedReferencesUnresolvable', 'rootPreserved', 'infoPreserved'].every((key) => proof[key] === true)
    || !['absent', 'permanent-preserved-changing-updated'].includes(proof.idPolicy)) fail('PDF_JAVASCRIPT_REMOVAL_OUTPUT_INVALID', 'The JavaScript-removal proof did not match the fixed compact-output contract.');
  return proof;
}
