...
import { registerApplicationShellEventsLifecycleTest } from './support/application-shell-events-lifecycle-test.js';
...
function shellEventTarget(name, contains = () => false, listenerLog = []) {
  ...
  return {
    ...
    addEventListener(eventName, listener, options) {
      registrations.push([name, eventName, listener, options]);
      listenerLog.push(['add', name, eventName, listener, options]);
      listeners.set(eventName, listener);
    },
    removeEventListener(eventName, listener, options) {
      removals.push([name, eventName, listener, options]);
...
registerApplicationShellEventsLifecycleTest();

test('application shell events use lifecycle file opening when tabs are unavailable', () => {
  const root = shellEventTarget('root');
  const calls = [];
  bindApplicationShellEvents({
    root,
    state: { dragging: true, analysis: { textPages: [] } },
    session: { subscribe: () => () => {} },
    lifecycle: { openFile: (file) => calls.push(file.name), dispose() {} },
    generation: { convertLocalFile() {} },
    document: shellEventTarget('document'),
    window: shellEventTarget('window'),
    render() {},
  });
  root.listeners.get('drop')(preventedEvent({
    dataTransfer: { files: [{ name: 'fallback.pdf', type: 'application/pdf' }] },
  }));
  assert.deepEqual(calls, ['fallback.pdf']);
});

test('application shell events validate dependencies before registering listeners', () => {
  const root = shellEventTarget('root');
  assert.throws(
    () => bindApplicationShellEvents({
      root,
    ...
    { name: 'TypeError', message: 'Application shell events require UI roots, session, controllers, and render.' },
  );
  assert.equal(root.registrations.length, 0);
});
