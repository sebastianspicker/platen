...
import { normalizedRegion, parsedTokens } from '../scripts/cli/parser-foundation.mjs';
...
test('CLI parser foundation preserves region and token validation contracts', () => {
  assert.deepEqual(normalizedRegion('0.1,0.2,0.5,0.3'), {
    x: 0.1, y: 0.2, width: 0.5, height: 0.3,
  });
  for (const value of ['0.1,0.2,0.5', '0.1,0.2,,0.3', '0.1,0.2,Infinity,0.3']) {
    assert.throws(
      () => normalizedRegion(value),
      { code: 'CLI_INVALID_OPTION', message: '--region must contain four comma-separated normalized numbers: X,Y,W,H.' },
    );
  }
  assert.throws(
    () => normalizedRegion('0.8,0.2,0.5,0.3'),
    { code: 'CLI_INVALID_OPTION', message: '--region must be a positive normalized rectangle fully inside the page.' },
  );

  const parsed = parsedTokens('inspect', ['input.pdf', '--output', 'report.json', '--structure']);
  assert.equal(parsed.values instanceof Map, true);
  assert.equal(parsed.flags instanceof Set, true);
  assert.deepEqual([...parsed.values], [['output', 'report.json']]);
  assert.deepEqual([...parsed.flags], ['structure']);
  assert.deepEqual(parsed.positionals, ['input.pdf']);
  assert.throws(() => parsedTokens('unknown', []), {
    code: 'CLI_UNKNOWN_COMMAND', message: 'Unknown command: unknown.',
  });
  assert.throws(() => parsedTokens('inspect', ['--output', 'one.json', '--output', 'two.json']), {
    code: 'CLI_INVALID_OPTION', message: 'Duplicate or empty option: --output.',
  });
  assert.deepEqual(parsedTokens('inspect', ['--']), {
    values: new Map(), flags: new Set(), positionals: [],
  });
  assert.throws(() => parsedTokens('inspect', ['--bogus']), {
    code: 'CLI_INVALID_OPTION', message: 'Option --bogus is not valid for inspect.',
  });
  assert.throws(() => parsedTokens('inspect', ['--output']), {
    code: 'CLI_INVALID_OPTION', message: 'Option --output requires a value.',
  });
  assert.deepEqual(
    parsedTokens('inspect', ['--', '--output', 'report.json']),
    { values: new Map(), flags: new Set(), positionals: ['--output', 'report.json'] },
  );
});
